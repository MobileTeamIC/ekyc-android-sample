...
class LogResultActivity : AppCompatActivity() {
   ...
   override fun onCreate(savedInstanceState: Bundle?) {
      ...
      setContentView(R.layout.log_result_activity)

      val logResults: ArrayList<LogResult>? =
         if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            intent.getParcelableArrayListExtra(SampleActivity.EXTRA_LOG_RESULT, LogResult::class.java)
         } else {
            @Suppress("DEPRECATION")
            intent.getParcelableArrayListExtra(SampleActivity.EXTRA_LOG_RESULT)
         }
